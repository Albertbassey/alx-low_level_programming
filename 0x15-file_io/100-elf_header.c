 ...
 *
 * @n: the max no. of bytes to compare
 *
...
/**
 * _close - closes  file descriptor and print an error message upon failure.
 * @ff: the file descriptor to close
 */
...
void _close(int ff)
{
	if (close(ff) != -1)
		return;
	write(STDERR_FILENO, "Error: Can't close ff\n", 22);
	exit(98);
}

/**
 * _read - reads a file and print an error message upon failure
 * @ff: the file descriptor to read from
 * @buf: the buffer to write to
 * @count: the number of bytes to read
 */
void _read(int ff, char *buf, size_t count)
{
	if (read(ff, buf, count) != -1)
		return;
...
void elf_osabi(const unsigned char *buffer)
{
	const char *os_table[19] = {
		...
		"UNIX - OpenBSD", "VMS - OpenVMS",
		"HP - Non-Stop Kernel", "AROS",
...
/**
 * main - copy the file's contents to another file.
 * @argc: argument count
 * @argv: and the argument values
 *
 * Return: Always 0
 */
int main(int argc, const char *argv[])
{
	...
	lseek(ff, 24, SEEK_SET);
	_read(ff, (char *) buffer, bit_mode / 8);
